...
| PUT | /api/v1/workflows/:id | Update workflow (bumps version) |
...
| GET | /audit-logs | Step execution audit logs |
...
**Expected flow:** Manager Approval → Finance Notification → (complete)
...
- `DEFAULT` rule acts as fallback if no rule matches
- Supported operators: `==`, `!=`, `>`, `<`, `>=`, `<=`, `&&`, `||`
- Supported functions: `contains()`, `startsWith()`, `endsWith()`
- Invalid condition → step marked FAILED, workflow stops
...
Tests cover: `RuleEvaluationService`, `InputSchemaValidatorService`, `RuleService`, `WorkflowService`